...
> from live web search and your own documents. Ask a harder one and it plans sub-questions,
> researches each, and merges the citations. It remembers you across sessions.
...
- **Ship two gears and defend the default**: quick answers most questions in seconds; deep plans, fans out, and merges. Knowing which a question deserves is the skill.
- **Make spend visible and gated**: every answer logs tokens and dollars; deep search sits behind a per-user daily cap enforced server-side.
...
`npm install && npm run dev` brings up both `501` services and the UI. The provided grader has
been exercised end to end against a stub backend, so when it fails you, it is failing on your
code and not on itself.
