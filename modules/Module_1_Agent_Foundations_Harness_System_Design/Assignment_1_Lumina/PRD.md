...
1. `PRODUCT_EVAL.md` (or PDF) generated by the `/fde-lumina-eval` skill, which runs the six gates (`check.mjs`, `bench.mjs`, `eval.mjs`) against the **deployed** gateway, attaches `reports/quality.json` and `reports/eval.json`, and records the two trajectories you read for P1.
