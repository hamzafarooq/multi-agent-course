...
Same FDE shape as before: a fixed contract, a provided UI that is the acceptance test, two backend services you own, an SLA you prove with a benchmark, a real deploy.
...
3. **Prove it by arithmetic.** `benchmark/bench.py` exits 0 against declared numbers in `sla.json`; no threshold is judged by a model.
...
*Scenario B — memory.* A week earlier Alex told LUMINA "I build in Python and want code examples, not prose." Today, in a new thread, "how do I call Tavily?" returns a Python snippet first. Alex opens the Memory panel, sees that stored preference, and deletes it.
...
| Must | Search results are **cached** (in-memory + SQLite) keyed by a hash of `(normalized query, provider)`, TTL default 6 h, `searchCached: true` in the `done` event when every search in the request was a hit. |
| Must | Page content is fetched and read (Tavily `extract`, or `trafilatura`/readability for SerpApi); the answer is synthesized from fetched text, not from search snippets alone. Snippet-only synthesis is visible in the trace and scored down. |
...
| Must | **Long-term memory:** durable facts and preferences per `X-User-Id`, stored in SQLite (or Postgres), each row `{id, text, source_thread, created_at}`. |
...
| Should | Memory recall is semantic (embedding similarity over memory rows), not just "inject all rows". Cap injected memory at ~10 rows / 1 000 tokens. |
...
| Pri | Requirement |
|---|---|
| Must | **Spaces:** `POST /spaces` creates a collection; `POST /spaces/{id}/documents` accepts PDF, Markdown, and plain text (≤ 25 MB), returns `202 {docId, status: "pending"}` and indexes on a background task — never in the request path. `GET /spaces/{id}/documents` shows `pending → parsing → embedding → indexed \| failed` with `pct`. |
...
| Must | One vector store for all Spaces, filtered by `spaceId` (Qdrant local/cloud, Chroma, or pgvector — learner's choice, named in `/health`). |
| Must | Hybrid retrieval (dense + BM25) with a re-rank step **or** a documented reason for skipping it; top-k and thresholds declared in config, not hard-coded. |
...
| Must | Pipeline: answer + its sources → a structured outline (JSON: title, 6–10 slides, each with bullets and the citation numbers it rests on) → rendered with `python-pptx`. The outline JSON is stored next to the file and returned by the API so grading can check grounding by arithmetic: every citation number on every slide must exist in the answer's `sources`. |
...
| Must | The stored artifact record carries `{model, size, costUsd, promptUsed}`; the file lives in object storage or a local `artifacts/` dir that is git-ignored. |
| Must | **Cost gate:** a per-user daily cap (`IMAGE_DAILY_CAP`, default 10) enforced at the AI service; over cap → `429 {error, resetsAt}`. `DRY_RUN=true` returns a placeholder image with `costUsd: 0` so the whole flow is testable without spend. |
...
Same split as every FDE project: the browser only ever talks to the gateway; keys live only in the AI service.

```
  ┌──────────────────────────────┐
  │  Web UI  (web/)              │  ← PROVIDED · React/Vite · the acceptance test
  ...
  └──────────────┬───────────────┘
                 ...
                 ▼
  ┌──────────────────────────────┐
  │  Node gateway  :8787         │  ← YOU · CORS · validate · rate-limit (429)
  │  request log · request id    │        serve web/ · proxy · SSE pass-through
  └──────────────┬───────────────┘
                 │  same contract
                 ▼
  ┌──────────────────────────────┐
  │  Python AI service  :8000    │  ← YOU · the loop and all seven tools
  │  loop · router · tools       │
  │  memory · RAG · artifacts    │
  └───┬──────┬──────┬──────┬─────┘
      ▼      ▼      ▼      ▼
   Search   LLM   Vector  SQLite/Postgres     Object storage (docs, decks, images)
  (Tavily/ (env-  store   (threads, memory,
  SerpApi)  swap) (Qdrant/ search cache,
                  Chroma/  artifacts)
                  pgvector)
```

**Why the gateway matters here more than in Live Translate:** SSE pass-through, per-user rate limiting, and the image cost gate are exactly the concerns you want on the edge, away from the keys.

**Async inside one service.** Document indexing and artifact generation run as background tasks in the AI service (FastAPI `BackgroundTasks` or a small worker thread). A real queue is not required; the *contract* (`202` + status polling + crash-safe status transitions) is. ARGUS swaps in Prefect against the same shape.
...
GET  /health                        → 200 { "status": "ok", "model": "…", "searchProvider": "tavily", "vectorStore": "qdrant", "ai": { "status": "ok" } }
...
| Table | Key fields |
|---|---|
| `threads` | `id, user_id, created_at, title` |
| `messages` | `id, thread_id, role, content, sources_json, done_json, created_at` |
| `memories` | `id, user_id, text, embedding, source_thread, created_at` |
| `spaces` / `documents` | `id, user_id, name` / `id, space_id, title, status, pct, pages, error, storage_uri` |
| `chunks` (in vector store) | `id, doc_id, space_id, text, locator_json, embedding, bm25 tokens` |
| `search_cache` | `key (sha256 of query+provider), results_json, expires_at` |
| `artifacts` | `id, user_id, thread_id, answer_id, kind, status, storage_uri, outline_json, prompt_used, model, cost_usd, error, created_at` |
| `requests` (log) | `request_id, user_id, route, status, ms, tokens_in, tokens_out, cost_usd, tool_calls, terminated` |

## 9 · Performance, SLA & cost (`benchmark/sla.json`)

Declared before the first run. `bench.py` exits non-zero on any miss; the grader runs it against the live deploy.
...
`bench.py` reports latency percentiles, grounding rate, recall, hit rate, cost per answer and per artifact, and projected monthly cost at the volume declared in `sla.json`, with and without the search cache.

## 10 · Observability

- One structured log line per request in the gateway (`method, route, status, ms, request_id, user_id`) and one per answer in the AI service (`request_id, tool_calls, terminated, tokens, cost_usd, search_cached, ttft_ms, latency_ms`).
...
3. **Errors surface, never swallowed.** Provider failure → `502` + log. No `try/except` that returns a plausible answer.
...
6. **Indexing and artifacts are async.** `202` in < 300 ms; status transitions are committed only after the work succeeds.
7. **Image spend is gated.** Daily cap enforced server-side; `DRY_RUN` exists; cost logged per image.
8. **Secrets from env, never committed.** `.env`, `*.db`, `artifacts/`, `.venv/`, `node_modules/` are git-ignored.
9. **Evidence over vibes.** Numbers in `PRODUCT_EVAL.md` come from a real `bench.py` run against the deployed app.
...
| Performance & SLA | 10 | auto | `bench.py` exits 0 | B1, B2, B3, A2, A3 |
...
| Deploy & docs | 5 | manual | Both services on Fly.io; UI works against the public gateway; one-command local run; `.env.example`; run notes | — |
...
`bench.py` writes `reports/eval.json` with exactly those metric names (`citationGrounding`, `recallAt5`, `retrievalRate`, `errorRate`), and the AI service writes one `runs/<requestId>.json` per answer:
...
A failed call **must** carry a non-empty `error` (A1). `terminated` is `"done"`, `"cap"`, or `"error"` — set explicitly at the call site, because no framework gives it to you.

### The gates

| Gate | Name | What runs | Blocks on |
|---|---|---|---|
| 0 | STATIC | lint, types, imports resolve, `git status` has no `.env` / `*.db` / `artifacts/` | any finding |
...
| 2 | RUN | `bench.py --smoke`: five queries complete inside `budget` with `terminated: "done"` | any run over budget or not done |
| 3 | TRAJECTORY | `check.mjs` over `runs/` → **A1, A2, A3, R2** | any error-severity fail |
| 4 | EVAL | full `bench.py` → `reports/eval.json` → **E1, E2** (grounding, recall, retrieval rate, error rate) + SLA percentiles in `sla.json` | any threshold missed |
...
1. AI service skeleton: `/health`, the loop with `web_search` + `fetch_page`, SSE `trace → sources → token → done`. Test with `curl -N`.
2. Search cache (memory + SQLite) and `searchCached`.
3. Gateway: CORS, `X-User-Id`, request id, structured log, SSE pass-through, serve `web/`.
4. Threads + messages persisted; follow-ups work.
5. Long-term memory: `save_memory` / `recall_memory`, `/memory` list + delete.
6. **Checkpoint demo:** cited streamed answer in the UI; a preference carries across threads.

**Week 2 — documents, artifacts, proof, deploy**
7. Spaces + async document indexing with page locators; router `auto`.
8. Deck artifact: outline JSON → `python-pptx`; grounding check on slide citations.
9. Image artifact with `gpt-image-1`, daily cap, `DRY_RUN`.
10. Write `expectations.json` **before** running the bench. Then `bench.py` green against `sla.json` and `check.mjs` exit ≤ 1 against `expectations.json`; fix what they catch. A first run that passes everything usually means the thresholds were set after seeing the scores.
11. Deploy both services to Fly.io; point the UI at the public gateway.
12. Run the eval skill → `PRODUCT_EVAL.md`; record the 60–90 s demo.

## 15 · Provided vs. built (what the course must ship before Week 1)

| Component | Status | Path |
|---|---|---|
| Web UI (React/Vite): query, streaming answer with citation chips, sources rail, thread list, memory panel, Spaces upload, deck/image actions with polling | ✅ Provided — **to be built by course staff** | `web/` |
| `benchmark/bench.py` + `sla.json` (latency, grounding, recall, cache, cost) | ✅ Provided — **to be built** | `benchmark/` |
| RAG gold set: a 30-question JSONL + a small corpus (3–5 PDFs, CC-licensed) | ✅ Provided — **to be built** | `eval/gold/` |
| `eval/eval.py` + `rubric.json` + the `/fde-lumina-eval` skill, running the six gates in order | ✅ Provided — **to be built** | `eval/`, `.claude/skills/` |
...
| Node gateway | 🔨 Learner | `backend/gateway-node/` |
| Python AI service: loop, tools, memory, RAG, artifacts | 🔨 Learner | `backend/ai-service-python/` |
...
**Assumptions:** learners have an LLM key, a Tavily or SerpApi key, and an OpenAI key for images; Fly.io free tier suffices for two small services; the provided UI can be built by staff in one week from this contract.
...
3. **Vector store default.** Qdrant (continuity with Module 3 and ARGUS) vs. Chroma (zero setup). Recommendation: Qdrant local via Docker, documented; any of the three accepted.
...
7. **Gold corpus content.** Which 3–5 public PDFs? Suggest arXiv RAG papers so the Space demo overlaps Module 3.
...
- **Dockerize** both services with `docker compose up`; GitHub Action running `bench.py` and `fly deploy` on green.

## 19 · Submission

1. `PRODUCT_EVAL.md` (or PDF) generated by the `/fde-lumina-eval` skill, which runs the six gates (`check.mjs`, `bench.py`, `eval.py`) against the **deployed** gateway, attaches `reports/quality.json` and `reports/eval.json`, and records the two trajectories you read for P1.
2. A **60–90 s recording:** a fresh question streams with citations; a memory carries into a new thread; a document question cites a page; the deck downloads and opens; an image generates and `/stats` shows its cost.
3. The repo with both `backend/` services; no `.env`, `*.db`, or `artifacts/` committed; a short "How I ran it" noting LLM, search, and vector-store choices.
