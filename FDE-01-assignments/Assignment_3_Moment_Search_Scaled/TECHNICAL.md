# TECHNICAL.md — ARGUS (Moment Search at Scale)

The build guide. [`README.md`](README.md) is the map — what you're building and in what
order. This is the detail: the architecture, the contract, every requirement, the gates,
and the failures that cost people the most time.

New to how this course structures an assignment? Read
[`START_HERE.md`](../../START_HERE.md) first — it explains
which files are written for you and which are written for your coding agent.
...
- [The API contract](#the-api-contract-extend-it-dont-break-it)
- [Build it — step by step](#build-it--step-by-step)
- [Performance, SLAs & scale](#performance-slas--scale)
- [Requirements checklist](#requirements-checklist)
- [Definition of Done](#definition-of-done--non-negotiables)
- [Grading](#grading-100-pts)
- [Stretch goals](#stretch-goals-bonus)
- [Submit](#submit)
...
- [ ] **Design document** — `DESIGN.md`, answered from
      [`DESIGN.template.md`](DESIGN.template.md) before you built the ingest pipelines.
      You are interviewed against it at submission.
...
Your `DESIGN.md` and the design interview are read alongside every row above: they're how a
grader tells a deliberate system from one that passed by following instructions. See
...
1. **Generate the evaluation with the skill.** In Claude Code, run
   **`/fde-momentsearch-scaled-eval`** (bundled in `.claude/skills/`). It runs the automated
   rubric + benchmark (including `--resilience`), performs a **live cross-source query on
   real media you don't control**, **interviews you on your `DESIGN.md` decisions**, and
   writes **`PRODUCT_EVAL.md`** at the assignment root. Export a PDF if you prefer
   (`md-to-pdf` skill or `pandoc`). Do **not** fabricate any recall numbers, latencies, or
   sample results — every value comes from an actual run.
...
2. **Submit `PRODUCT_EVAL.md` (or the PDF)** and a **60–90s screen recording**: one
   natural-language query landing a cited moment in a video, a cited page in a paper, and a
   cited slide in a deck — then the Prefect run view during a backfill while a search still
   returns fast.
3. Push your fork with the two new ingest paths implemented, and your completed `DESIGN.md`.
   Do **not** commit `.env`, `.venv/`, model caches, or media/PDF files. Add a short
   **"How I ran it"** to your README: your LLM/embedding provider, and where you deployed.
